...
# Enable & start tuning-cycle timer (runs weekly after scorecard)
if [[ -f "${SYSTEMD_DIR}/tuning-cycle.timer" ]]; then
  systemctl enable --now tuning-cycle.timer
  ok "Tuning-cycle timer enabled (runs every Sunday 09:20)."
fi
