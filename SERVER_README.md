...
- `UPTREND_ENTRY_MAX_RSI_BY_COIN`, `UPTREND_ENTRY_MIN_BUY_PROBA_BY_COIN`, `UPTREND_ENTRY_MAX_SELL_PROBA_BY_COIN`, and `UPTREND_ENTRY_MIN_PROBA_EDGE_BY_COIN` allow coin-specific entry tuning without loosening the global gate; current defaults only nudge `TRX` and `ONDO`, while `XDC` still stays behind the stronger sell-proba guard
